...
const logger = createLogger({ context: 'ProcurementError' });
...
/**
 * Error Boundary for Procurement Module
 *
 * Catches and handles errors in the procurement module with module-specific
 * context and recovery options.
 */
export default function ProcurementError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    // Log error to monitoring service
    logger.error('Procurement module error', {
    ...
    Sentry.withScope((scope) => {
      ...
      scope.setTag('errorBoundary', 'procurement-module');
      if (error.digest) {
      ...
      scope.setContext('moduleInfo', {
        module: 'procurement',
        description: 'Error in procurement processing',
      });
      ...
      >
        <Stack spacing={3} alignItems="center">
          ...
          <Typography variant="h4" component="h1" fontWeight="bold" textAlign="center">
            Procurement Module Error
          </Typography>

          <Alert severity="error" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred while processing your procurement request. Your data has not been
