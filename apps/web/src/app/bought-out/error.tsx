...
const logger = createLogger({ context: 'BoughtOutError' });
...
/**
 * Error Boundary for Bought Out Module
 */
export default function BoughtOutError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Bought Out module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'bought-out');
      scope.setTag('errorBoundary', 'bought-out-module');
      if (error.digest) {
      ...
      <Paper elevation={3} sx={{ p: 4, maxWidth: 700, width: '100%' }}>
        <Stack spacing={3} alignItems="center">
          ...
          <Typography variant="h4" component="h1" fontWeight="bold" textAlign="center">
            Bought Out Items Error
          </Typography>
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the bought out items module. Please try refreshing the page.
            </Typography>
          </Alert>
