...
const logger = createLogger({ context: 'SuperAdminError' });
...
/**
 * Error Boundary for Super Admin Module
 */
export default function SuperAdminError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Super Admin module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'super-admin');
      scope.setTag('errorBoundary', 'super-admin-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the super admin module. Please try refreshing the page.
            </Typography>
          </Alert>
