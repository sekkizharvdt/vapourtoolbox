...
const logger = createLogger({ context: 'DocumentsError' });
...
/**
 * Error Boundary for Documents Module
 */
export default function DocumentsError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Documents module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'documents');
      scope.setTag('errorBoundary', 'documents-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the documents module. Please try refreshing the page.
            </Typography>
          </Alert>
