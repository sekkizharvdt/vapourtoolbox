...
const logger = createLogger({ context: 'GuideError' });
...
/**
 * Error Boundary for Guide Module
 */
export default function GuideError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Guide module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'guide');
      scope.setTag('errorBoundary', 'guide-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred loading the user guide. Please try refreshing the page.
            </Typography>
          </Alert>
