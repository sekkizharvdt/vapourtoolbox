...
const logger = createLogger({ context: 'EntitiesError' });
...
/**
 * Error Boundary for Entities Module
 */
export default function EntitiesError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Entities module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'entities');
      scope.setTag('errorBoundary', 'entities-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the entities module. Please try refreshing the page.
            </Typography>
          </Alert>
