...
/**
 * Full Design Panel — Iterative Heat Exchanger Sizing
 *
 * Extracted from HeatExchangerClient for the unified heat exchanger calculator.
 */
...
interface FullDesignPanelProps {
  active: boolean;
}

export function FullDesignPanel({ active }: FullDesignPanelProps) {
  ...
  const handleLoad = useCallback((inputs: Record<string, unknown>) => {
    ...
    if (typeof inputs.satTemp === 'string' && inputs.satTemp) {
      setShellSideCondensing((prev) => ({ ...prev, saturationTemp: inputs.satTemp as string }));
    }
    ...
    if (!inputs.exchangerType) setExchangerType('CONDENSER');
    setDesignResult(null);
    setDesignError(null);
  }, []);
