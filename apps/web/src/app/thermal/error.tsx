...
const logger = createLogger({ context: 'ThermalError' });
...
/**
 * Error Boundary for Thermal Module
 */
export default function ThermalError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Thermal module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'thermal');
      scope.setTag('errorBoundary', 'thermal-module');
      if (error.digest) {
      ...
      <Paper elevation={3} sx={{ p: 4, maxWidth: 700, width: '100%' }}>
        <Stack spacing={3} alignItems="center">
          ...
          <Typography variant="h4" component="h1" fontWeight="bold" textAlign="center">
            Thermal Calculations Error
          </Typography>
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the thermal calculations module. Please try refreshing the page.
            </Typography>
          </Alert>
