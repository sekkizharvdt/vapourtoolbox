...
const logger = createLogger({ context: 'FeedbackError' });
...
/**
 * Error Boundary for Feedback Module
 */
export default function FeedbackError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Feedback module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'feedback');
      scope.setTag('errorBoundary', 'feedback-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the feedback module. Please try refreshing the page.
            </Typography>
          </Alert>
