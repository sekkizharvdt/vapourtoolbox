...
const logger = createLogger({ context: 'FlowError' });
...
/**
 * Error Boundary for Flow Module
 */
export default function FlowError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Flow module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'flow');
      scope.setTag('errorBoundary', 'flow-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the flow module. Please try refreshing the page.
            </Typography>
          </Alert>
