...
const logger = createLogger({ context: 'UsersError' });
...
/**
 * Error Boundary for Users Module
 */
export default function UsersError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Users module error', {
    ...
    Sentry.withScope((scope) => {
      ...
      scope.setTag('errorBoundary', 'users-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the users module. Please try refreshing the page.
            </Typography>
          </Alert>
