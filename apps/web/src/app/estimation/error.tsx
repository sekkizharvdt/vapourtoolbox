...
const logger = createLogger({ context: 'EstimationError' });
...
/**
 * Error Boundary for Estimation Module
 */
export default function EstimationError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Estimation module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'estimation');
      scope.setTag('errorBoundary', 'estimation-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the estimation module. Please try refreshing the page.
            </Typography>
          </Alert>
