    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="warning">
          You do not have permission to view payment batches.
        </Alert>
      </Box>
    );
  }
          ...
          <Box component="ul" sx={{ mt: 1, mb: 0, pl: 2 }}>
            {crossProjectPayments.map((cp) => (
              <li key={cp.payment.id}>
                <Typography variant="body2">
                  {cp.payment.entityName} ({formatCurrency(cp.payment.amount)}) -{' '}
                  Loan from <strong>{cp.lendingProjectName}</strong> to{' '}
