...
const logger = createLogger({ context: 'ProposalPreviewPage' });

interface PreviewPageProps {
  params: Promise<{ id: string }>;
}

export default function ProposalPreviewPage({ params }: PreviewPageProps) {
  ...
  return (
    <Container maxWidth="lg">
      ...
      <Paper sx={{ p: 4, mb: 3 }}>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Prepared: {formatDate(proposal.preparationDate)} | Valid until: {formatDate(proposal.validityDate)}
          </Typography>
        </Box>
                    ...
                    ([phase, items]) => (
                      <Box key={phase} sx={{ mb: 2 }}>
                        <Typography variant="subtitle2" color="text.secondary" sx={{ mb: 1 }}>
                          {phase === 'UNASSIGNED' ? 'General' : PROJECT_PHASE_LABELS[phase as keyof typeof PROJECT_PHASE_LABELS]}
                        </Typography>
                        <List dense>
                          {items.map((item) => (
                      ...
                      <TableBody>
                        {proposal.scopeMatrix.supply.map((item) => (
                          <TableRow key={item.id}>
                            <TableCell>{item.itemNumber}. {item.name}</TableCell>
            ...
            <CardContent>
              ...
              <Box sx={{ mt: 2 }}>
                <Typography variant="body2">
                  <strong>Validity:</strong> {proposal.pricingConfig.validityDays} days from date of issue
                </Typography>
      ...
      <Dialog open={submitDialogOpen} onClose={() => setSubmitDialogOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to mark this proposal as submitted to the client?
            This will change the status to &quot;Submitted&quot; and record the submission date.
          </DialogContentText>
