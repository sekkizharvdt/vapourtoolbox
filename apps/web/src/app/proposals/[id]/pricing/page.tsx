...
const logger = createLogger({ context: 'PricingEditorPage' });

interface PricingEditorProps {
  params: Promise<{ id: string }>;
}

export default function PricingEditorPage({ params }: PricingEditorProps) {
    ...
    <Container maxWidth="lg">
      ...
      {!hasEstimation ? (
        <Alert severity="warning" sx={{ mb: 3 }}>
          No estimation data found. Please link BOMs to scope items in the Scope Matrix before configuring pricing.
          <Button
            size="small"
            sx={{ ml: 2 }}
