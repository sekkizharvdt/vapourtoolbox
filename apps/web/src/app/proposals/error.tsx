...
const logger = createLogger({ context: 'ProposalsError' });
...
/**
 * Error Boundary for Proposals Module
 */
export default function ProposalsError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    logger.error('Proposals module error', {
    ...
    Sentry.withScope((scope) => {
      scope.setTag('module', 'proposals');
      scope.setTag('errorBoundary', 'proposals-module');
      if (error.digest) {
        ...
        <Stack spacing={3} alignItems="center">
          ...
          <Alert severity="warning" sx={{ width: '100%' }}>
            <Typography variant="body1">
              An error occurred in the proposals module. Please try refreshing the page.
            </Typography>
          </Alert>
