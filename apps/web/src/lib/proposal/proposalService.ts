...
export async function recordApprovalAction(
  ...
  try {
    ...
    const newApprovalHistory = [...proposal.approvalHistory, approvalRecord];

    let newStatus: ProposalStatus = proposal.status;
    if (action === 'APPROVED') {
      newStatus = 'APPROVED';
    } else if (action === 'REJECTED' || action === 'REQUESTED_CHANGES') {
    ...
    logger.info('Approval action recorded', { proposalId, action, approver: approverUserName });
  } catch (error) {
    logger.error('Error recording approval action', { proposalId, error });
    throw error;
  }
}
 ...
 */
export async function submitProposalToClient(
  db: Firestore,
  proposalId: string,
  userId: string,
  userName: string,
  ...
  try {
    ...
    if (pdfUrl) {
      updates.generatedPdfUrl = pdfUrl;
    }
    ...
    const proposal = await getProposalById(db, proposalId);
    if (proposal) {
      await updateDoc(doc(db, COLLECTIONS.ENQUIRIES, proposal.enquiryId), {
    ...
    logger.info('Proposal submitted to client', { proposalId });
  } catch (error) {
    logger.error('Error submitting proposal to client', { proposalId, error });
    throw error;
  }
}
...
export async function acceptProposal(
  ...
  try {
    ...
    logger.error('Error accepting proposal', { proposalId, error });
    throw error;
  }
}
...
export async function rejectProposal(
  ...
  try {
    ...
    logger.info('Proposal rejected', { proposalId, rejectionReason });
  } catch (error) {
