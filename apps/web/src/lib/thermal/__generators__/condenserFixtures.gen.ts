...
/** Default seawater temperature rise across the condenser, K. See seawaterOutletRule. */
const SEAWATER_RISE_DEFAULT_K = 5;
...
const CASE_SPECS: CaseSpec[] = [
  ...
  },
  {
    id: 'cond-06',
    note:
      'ADDED v2. Explicit condenserSWOutlet giving an 8 K seawater rise — the TOP of the range a ' +
      'designer uses. Without a case like this the rise was 5.00 K across the whole grid and a ' +
      'settable input was indistinguishable from a constant, the same blind spot the vent-gas ' +
      'saturation ceiling had in the vacuum fixture.',
    input: { ...BASE, condenserSWOutlet: 38 },
  },
  {
    id: 'cond-07',
    note:
      'ADDED v3. A 7 K rise, the middle of the practical range. With 5, 7 and 8 K on the grid a ' +
      'consumer can see the span a designer actually works in rather than inferring one from ' +
      'two endpoints.',
    input: { ...BASE, condenserSWOutlet: 37 },
  },
];
...
function buildCase(spec: CaseSpec) {
  ...
  return {
    ...
    expected: {
      ...
      // Components, so `dutyKW` never has to be inverted. See dutyComposition.
      dutyBreakdownKW: {
        condensing: round(c.dutyBreakdown.condensingKW, 3),
        distillateCooling: round(c.dutyBreakdown.distillateCoolingKW, 3),
        condensateCooling: round(c.dutyBreakdown.condensateCoolingKW, 3),
      },
      ...
      tubeWallThicknessMm: c.tubeWallMM,
      tubeMaterial: c.tubeMaterial,
      tubeConductivityWmK: c.tubeConductivityWmK,
      tubeIDmm: round(c.tubeOD - 2 * c.tubeWallMM, 4),
      ...
      seawaterOutletTempC: round(
        spec.input.condenserSWOutlet ?? spec.input.seawaterTemperature + SEAWATER_RISE_DEFAULT_K,
        3
      ),
    },
  };
...
export function buildCondenserFixturePayload() {
  ...
  return {
      ...
      v4: {
        added: [
          'expected.tubeWallThicknessMm, .tubeMaterial, .tubeConductivityWmK, .tubeIDmm — the ' +
            'resistance-network constants, per case and taken from the result',
          'seawaterOutletRule.cascadeEffect — the measured plant-level consequence',
          'workflow — the flash chamber and condenser are separate calculators BY DESIGN',
        ],
        removed: [
          'resistanceNetwork.tubeWallThicknessMm and .tubeConductivityWmK — they were WRONG ' +
            'and are replaced by per-case values from the result',
        ],
        changed: [],
        note:
          'CORRECTION. v3 published a 1.0 mm tube wall and k = 138, which are the EVAPORATOR ' +
          'defaults; the condenser is 17 x 0.4 mm titanium at k = 21. Closing the network from ' +
          'the v3 constants gives U about 2.3% low in one direction — the 1.8-3.7% the simulator ' +
          'session measured. Their finding was correct and the cause was a published constant, ' +
          'not an undocumented route in the calculator. No computed value in the fixture moves; ' +
          'only the constants describing how it was computed.',
      },
      v3: {
        added: [
          'case cond-07 — a 7 K seawater rise, the middle of the practical range',
          'seawaterOutletRule.practicalRangeK and .designBasis',
        ],
        removed: [],
        changed: [],
        note:
          'The seawater rise is a DESIGN CHOICE: 5 K default, 7-8 K as the duty requires, ' +
          'confirmed by the domain user. v2 had only 5 and 8 K; v3 spans the range so a consumer ' +
          'sees where a designer actually works rather than inferring it from two endpoints. No ' +
          'existing case moves.',
      },
      v2: {
        added: [
          'expected.dutyBreakdownKW — the three terms dutyKW is made of',
          'expected.seawaterOutletTempC',
          'dutyComposition, seawaterOutletRule and resistanceNetwork sections',
          'case cond-06 — an 8 K seawater rise',
        ],
        removed: [],
        changed: [],
        note:
          'Prompted by the simulator session finding that dutyKW is NOT ' +
          'vapourFlowTPerH x h_fg(vapourTempC) — it implies 2-7% more condensing flow. The cause ' +
          'is two sensible-cooling terms plus a vent fraction and a wider enthalpy span, not ' +
          'extra vapour, so the excess is heat and not mass. They also found the resistance ' +
          'network could not be closed from what v1 published: fouling, tube wall and ' +
          'conductivity account for 26-36% of 1/U and were absent. No existing value moves.',
      },
      v1: {
    ...
    dutyComposition: {
      relation: 'dutyKW = condensing + distillateCooling + condensateCooling',
      whyNotVapourTimesLatentHeat:
        'dutyKW is NOT vapourFlowTPerH x h_fg(vapourTempC). That relation implies a condensing ' +
        'flow 2-7% too large, varying by case. Three effects, pulling in opposite directions: ' +
        '(1) the condensing term is net of the vent fraction, so it uses ' +
        'm_vap x (1 - VENT_FRACTION) and is 3% SMALLER; (2) it subcools to the distillate outlet ' +
        'temperature rather than to the vapour temperature, so the enthalpy span is ' +
        'h_vap(T_vap) - h_liq(T_distOut), WIDER than h_fg(T_vap); (3) two sensible terms add ' +
        'heat from the distillate cascade and the first-effect steam condensate.',
      theExcessIsHeatNotVapour:
        'CRITICAL for a coupled model. The 2-7% excess is NOT additional condensing flow. Both ' +
        'sensible terms cool streams that arrive as LIQUID — already condensed upstream — so ' +
        'treating the excess as extra vapour would add mass that is not there. Take the ' +
        'condensing load from dutyBreakdownKW.condensing, and the sensible terms as what they ' +
        'are.',
      ventFraction: VENT_FRACTION,
      condensateCoolingIsZeroHere:
        'condensateCooling is 0.0 kW in every case on this grid, because first-effect steam ' +
        'condensate is not routed to the final condenser in these configurations. It is a real ' +
        'term in the model, not a placeholder — do not assume it stays zero for a different ' +
        'condensate extraction setting.',
    },

    seawaterOutletRule: {
      relation: 'T_sw,out = condenserSWOutlet ?? (T_sw,in + 5); T_vap,last = T_sw,out + approach',
      defaultRiseK: SEAWATER_RISE_DEFAULT_K,
      practicalRangeK: [5, 8],
      designBasis:
        'Confirmed by the domain user 2026-08-04: 5 K is the MED default and a designer raises it ' +
        'to 7 or 8 K as the duty requires. It is a DESIGN CHOICE, not a physical constant and not ' +
        'a derived quantity — which is why it is an optional input rather than a computed one. ' +
        'Cases on this grid span 5 K (cond-01..05), 7 K (cond-07) and 8 K (cond-06).',
      isADefaultNotAConstant:
        'v1 of this fixture left condenserSWOutlet unset in every case, so the rise was 5.00 K ' +
        'everywhere and a settable input looked like a constant. Seawater FLOW is derived from ' +
        'it, not independent: m_sw = Q / (cp_sw x dT_sw). A dynamic model that varies cooling ' +
        'water flow freely has left the regime these cases describe — here the flow follows from ' +
        'the duty and the chosen rise.',
      cascadeEffect:
        'MEASURED, and it is the dominant consequence of this input. Raising the rise from 5 to ' +
        '8 K moves the last effect from 39.63 to 42.64 °C and its pressure from 66.7 to ' +
        '78.8 mbar, which COMPRESSES the available cascade from 19.43 to 16.92 K and costs ' +
        '22% MORE evaporator area (507.6 to 619.4 m2). Condenser duty moves only 0.6%. A cold-end ' +
        'choice whose largest effect is three steps upstream, in a different piece of equipment.',
      alsoSetsTheCascade:
        'T_sw,out feeds the last-effect vapour temperature via + condenserApproach, so it moves ' +
        'the whole temperature cascade, not just the condenser. That is why cond-04 (widened ' +
        'approach) differs from cond-06 (raised outlet) despite both changing the cold end.',
    },

    resistanceNetwork: {
      note:
        'The constants needed to CLOSE 1/U from the two film coefficients. Without them a U ' +
        'disagreement can be seen but not attributed, and gateGuidance asks for exactly that ' +
        'attribution. These account for roughly 26-36% of 1/U, so they are not a rounding term.',
      relation:
        '1/U = 1/h_shell + R_f,shell + (OD x ln(OD/ID))/(2 x k_wall) + R_f,tube x (OD/ID) + (1/h_tube) x (OD/ID)',
      seawaterFoulingM2KW: DEFAULT_FOULING_SEAWATER,
      distillateFoulingM2KW: DEFAULT_FOULING_DISTILLATE,
      correction:
        'v3 PUBLISHED THE WRONG TUBE CONSTANTS. It gave a 1.0 mm wall and k = 138 W/(m·K) — the ' +
        'EVAPORATOR defaults (Al 5052) — while condenser tubes are 17 x 0.4 mm TITANIUM with ' +
        'k = 21. A consumer closing the network from those figures computes U about 2.3% low, ' +
        'in one direction, which is what the simulator session measured (1.8-3.7%). Their ' +
        'observation that referring the tube-side coefficient to the mean diameter narrowed the ' +
        'gap was a real signal pointing at the right term: the wrong wall thickness also gives ' +
        'the wrong OD/ID referral ratio (1.133 against 1.049), and a mean-diameter referral ' +
        'partly cancels it. The constants are now taken PER CASE from the calculator result ' +
        'rather than assumed, so they cannot drift again.',
      perCaseConstants:
        'tubeWallThicknessMm, tubeConductivityWmK and tubeMaterial are on each case under ' +
        'expected, not here — they are outputs of the design, not fixture-level constants.',
      note_whichSideIsWhich:
        'In the CONDENSER seawater is tube-side and condensing distillate is shell-side, so ' +
        'R_f,tube is the seawater value and R_f,shell the distillate one. In a MED EFFECT it is ' +
        'the other way round — steam condenses inside the tubes and brine sprays outside. The ' +
        'asymmetry is the geometry, not an inconsistency.',
    },

    workflow: {
      flashChamberAndCondenserAreSeparate:
        'BY DESIGN, not a gap. A flash chamber directly connected to a condenser is rare in ' +
        'practice, so Vapour Toolbox keeps them as separate calculators and the engineer links ' +
        'them by hand through the saturation temperature: choose the operating pressure, read ' +
        'the saturation temperature, size the chamber with flashChamberCalculator, then take the ' +
        'vapour flow and temperature into heatDutyCalculator and heatExchangerSizing as a latent ' +
        'duty. heatExchangerSizing handles sensible, latent or combined duty and is used for MED ' +
        'condensers and preheaters.',
      whatThatMeansForACoupledModel:
        'The flash chamber takes its operating pressure as an INPUT; it is not derived from a ' +
        'condenser energy balance. A coupled model where flash pressure emerges from condensation ' +
        'is doing something neither reference here can check — that is a difference in what the ' +
        'tools are for, not a defect in these. Correctness there rests on conservation and the ' +
        "model's own invariants, and a run report should say so.",
    },
