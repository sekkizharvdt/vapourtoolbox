/**
 * Calculator → SSOT Sync
 *
 * Persists the output of a calculator → SSOT generator (`medDesignGenerator`,
 * `flashChamberGenerator`) into a project's SSOT registers, merging with
 ...
 *   Same-source records     → calculated fields refreshed, EXCEPT any field
 *                             listed in `provenance.manualOverrides`
 *   Other-source records    → ignored entirely; a MED regeneration must not
 *                             touch a flash chamber's geometry, or vice versa
 ...
 *
 * ── Two phases ───────────────────────────────────────────────────────────
 * `planSSOTSync()` reads and decides (no writes) so the dialog can show
 * exactly what will happen. `applySSOTSync()` executes an approved plan.
 */
...
/**
 * The shape every calculator → SSOT generator produces. Declared here rather
 * than imported from one generator so the sync layer does not depend on the MED
 * bridge in particular — it serves the flash chamber generator too.
 */
export interface SSOTGeneration {
...
const logger = createLogger({ context: 'ssotSync' });
...
export function planRegister<TExisting extends { id: string }, TInput extends object>(
  ...
  accessors: {
    ...
    /** Which calculator produced an existing record */
    existingSource: (record: TExisting) => string | undefined;
    /** The calculator this sync is running for */
  ...
  for (const record of existingRecords) {
    ...
    // Records generated by a DIFFERENT calculator are none of this sync's
    // business. Without this, syncing a flash chamber into a project that also
    // carries MED registers reports every MED record as an orphan — merely
    // because this generation does not contain it. Orphans are never
    // auto-deleted, so that was a misreport rather than data loss, but it is
    // the kind of misreport that gets acted on by hand.
    //
    // MANUAL records stay in the map deliberately: they must still be matched
    // so they can be reported as skipped, otherwise a hand-entered record with
    // the same key would be duplicated by a create.
    const recordSource = accessors.existingSource(record);
    if (recordSource && recordSource !== 'MANUAL' && recordSource !== accessors.source) continue;
...
const sourceOf = (record: { provenance?: { source?: string } }): string | undefined =>
...
/**
 * Provenance to write for a generated record.
 *
 * Every generator stamps provenance, so the fallback is defensive — but it takes
 * the sync's own source rather than defaulting to MED_DESIGN, which would label
 * a flash chamber record as MED and hand it to the wrong regeneration.
 */
const generatedProvenanceFor =
  (source: SSOTRecordSource) =>
  (input: { provenance?: SSOTProvenance }): SSOTProvenance =>
 ...
 */
export async function planSSOTSync(
  projectId: string,
  generation: SSOTGeneration,
  source: SSOTRecordSource
): Promise<SSOTSyncPlan> {
  logger.debug('planSSOTSync', { projectId, source });
  ...
  const streams = planRegister<ProcessStream, ProcessStreamInput>(
    ...
    generation.streams,
    {
      ...
      existingSource: sourceOf,
      source,
      generatedKey: generatedKeyOf,
      generatedProvenance: generatedProvenanceFor(source),
      label: (s) => s.lineTag,
 ...
 */
export async function applySSOTSync(
  plan: SSOTSyncPlan,
  userId: string,
  accessCheck?: SSOTAccessCheck
): Promise<SSOTSyncResult> {
  const { projectId } = plan;
  logger.debug('applySSOTSync', { projectId });
...
export function summarisePlan(plan: SSOTSyncPlan): SyncPlanTotals {
