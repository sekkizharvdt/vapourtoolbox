...
**Status:** ACTIVE — decisions locked 2026-07-07. **Phases 0, 1, 2, 3, 4, 5, 7 + C5 SHIPPED & DEPLOYED** (0: `ae8ed6eb` shapes; 1: A1 BOM→proposal cost basis, A2 pricing-feed leaks, A3 thermal→BOM export; 2: `c75976cd` execution loop B1–B5 + a new Order Acceptance charter section (see below); 3+C5: `c8308439` money integrity + fiscal close; 4: `4938e2ce` reliability+hardening; 5: `c3cd6b84` thermal correctness; 7: `84893d0c` catalog facade — all verified, tests green, balances recalculated in prod). **REMAINING: Phase 6 (scope-aware pricing + BOM→PR)**; ongoing: App Check enforcement flip after metrics settle, Track F opportunistic. Recorded follow-ups: comp-off grant-level tracking, heat-duty/heat-transfer dead-client retirement, catalog legacy id-triplet retirement after backfill.

**Phase 2 acceptance test (Desolenator PO26XP062901) surfaced a real gap beyond B1–B5's scope**: `convertProposalToProject` copies the submitted proposal into the charter verbatim, with no mechanism to reconcile against what the customer actually signed — the live Desolenator project carried the proposal's 3-week schedule and unstructured payment terms instead of the signed agreement's 2 weeks / 60-day terms / 15% retention / 8-item deliverables register (see [PO26XP062901 analysis](2026-07-03-po26xp062901-proposal-vs-agreement.md)). Closed by a new **Order Acceptance** charter section (`c75976cd`) — captures the signed document's schedule/payment terms/retention/deliverables/key-personnel as a structured, approved record that overwrites the charter's authoritative fields on approval. Works retroactively (no special-case code) so it applies to Desolenator as-is. **Desolenator's actual signed terms still need to be entered through this UI** — that's a data-entry/business decision for a human, not something to script.
