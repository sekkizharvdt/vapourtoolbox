...
| Rev    | Date       | Change                                                                                                                                                                                |
| ------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| r1     | 2026-07-29 | Initial draft. **Not retained** — edited in place before revision numbering was adopted, and never committed, so it is unrecoverable.                                                 |
| **r2** | 2026-07-29 | Resolves the r1 review. Four blocking contradictions fixed (pressure state vs algebraic, quasi-steady vs pull-down, DAE/inner-solve, stiffness), plus 13 further findings. See below. |

### What changed in r2

**Blocking fixes**

- **§2.3 rewritten.** The r1 text said the vapour space was quasi-steady _and_ that
  pressure becomes a state — contradictory, and as written it made vacuum pull-down
  (rung 3) unmodellable, since pull-down _is_ an inventory integration. Resolved by
  splitting the vapour phase: **steam quasi-steady, NCG a state, total pressure explicit**.
- **No DAE, no inner solve.** Stated explicitly, with a standing rule to promote an unknown
  to a state rather than add a Newton solve inside the derivative function.
- **§2.3.1 stiffness claim withdrawn.** Two orders of magnitude is not stiff; the r1
  "1-second steps / ~7,200 steps" cost estimate was meaningless because `solve_ivp` is
  adaptive. Now: measure with `LSODA`, do not assert.

**Correctness**

- **§6.8 `cold_flooded` was internally inconsistent** — `P_ncg = P_atm` gives
  `P_total ≈ 1055 mbar`, not atmospheric. Corrected to `P_ncg = P_atm − P_sat`.
- **§2.3 NCG temperature** was unqualified. Now stated: thermally slaved to the liquid
  surface, with what that gives up.
- **§7.2.1 rung 4 gate was wrong** if ambient heat loss was enabled at the same rung — loss
  flows through the metal and genuinely moves the equilibrium. Rung 4 split into 4 (thermal
  mass, loss disabled) and 4b (loss, separately gated).
- **§7.2.1 rung 3 gate replaced.** `vacuumSystemCalculator` does not predict a steady-state
  pressure from in-leakage — it takes suction pressure as an input. It _does_ compute an
  evacuation curve, which is a better, transient reference. Verified against the source.

**Specification gaps closed**

- §5.2.1 defines the dry state concretely (freeze derivatives, hold `m_min`, no change to
  the state-vector shape) and requires conservation to carry the residual.
- §5.2.2 covers level-map conditioning; §6.6 covers event conditioning.
- §4.2 adds the vapour-space discharge boundary, without which rung 2 has no closure.
- §6.1 fixes the run contract at three artifacts, with numerical settings inside the
  parameter set; §6.1.1 adds result versioning; §6.1.2 fixes the serialisation format.
- §7.2.1 states rung 2's round-trip check in order, so it is not circular.
- §2.2/§7.1 order the property-verification hierarchy: paper first, MIT reference second,
  TypeScript agreement third. §2.2.1 names BPE as the highest-risk property.
- §6.2 scenario keys carry units in names, removing the need for a units library.
