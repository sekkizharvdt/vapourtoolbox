...
**Status:** COMPLETE — Phase 1 shipped tactically in `c86c0c38`/`ebec0246`/`c4904a8e` (2026-06); Phases 2–3 shipped in `84893d0c` (2026-07-13): CatalogRef + catalogService + CatalogPickerDialog + CATALOG_TAXONOMY, PR/Quotes/BOM consumers migrated, AddBOMItemDialog's bought-out-as-material hack retired. Pulled ahead of completion-plan A2/A5 by user decision; when those land they consume this facade. Remaining opportunistic (Track F): absorb the per-kind list UIs into one searchCatalog-driven list, retire legacy id triplets after backfill.
