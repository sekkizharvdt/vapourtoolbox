...
- **ExpenseClaim → reimbursement**: verified 2026-04-21 — no reimbursement-payment flow exists (ExpenseClaim is a first-class transaction type, but nothing generates a follow-on DIRECT_PAYMENT carrying a `sourceExpenseClaimId` link). When the flow is built, the generated payment must carry `sourceExpenseClaimId` + `claimantName` + `approvedAmount`.
